 ...
 * Allows users to exchange tokens using De¹ API
 */
const Swap = () => {
  ...
  const handleSwap = async () => {
    ...
    try {
      ...
      // Get swap quote from De¹ API
      let url = `https://open-api.de1.exchange/v4/${chain.chainId}/swap?${Object.entries(params).map(([key, value]) => `${key}=${value}`).join('&')}`
      let res = await axios.get(url);

      const { inToken: inTokenObj, inAmount, data, to, gasPrice: swapGasPrice } = res.data.data;
      ...
      if (!isNativeToken(inTokenAddr, chain.chainId)) {
        ...
        await checkTokenApprove(inTokenAddr, to, fromAmountDecimals, gasPrice, approveAmount);
      } else {
  ...
  /**
   * Fetch available tokens from De¹ API
   */
  const getTokens = async () => {
    let url = `https://open-api.de1.exchange/v4/${chain.chainId}/tokenList`
  ...
  const getQuote = async (value) => {
    ...
    setIsLoading(true);
    try {
      let url = `https://open-api.de1.exchange/v4/${chain.chainId}/quote?inTokenAddress=${inToken.address}&outTokenAddress=${outToken.address}&amountDecimals=${(value || fromAmount) * 10 ** inToken.decimals}&slippage=${slippage}&gasPrice=${gasPrice}`
      const { data } = await axios.get(url);
      console.log(data);
      setQuote(data.data);
